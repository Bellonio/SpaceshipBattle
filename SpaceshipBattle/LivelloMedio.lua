...
function LivelloMedio:init()
	...
	self.timerSparoNave = 1.3
	self.countdownSparoNave = 1.3
	
	self.timerSparoBoss = 0.4
	self.countdownSparoBoss = 1.3
	
	self.timerPowerUp = 0
	self.countdownPowerUp = 2.5
...
function LivelloMedio:update(dt)
		...
		if self.boss.padre.vita > 0 and self.nave.padre.vita > 0 then
			...
			if self.timerPowerUp > self.countdownPowerUp then
				...
				self.timerPowerUp = 0
					...
					--ora bisognera' aspettare 1.5 secondi in piu' per un powerup
				self.countdownPowerUp = self.countdownPowerUp + 1.5
			...
			if self.timerSparoBoss > self.countdownSparoBoss then
				...
				suoni["sparoBoss"]:play()
					...
					--nel livello medio il boss spara due colpi per volta, ma il tipo di colpo che spara e' quello successivo a quello che usa la nave. Angolati di 30°.
				
				self.colpiBoss[self.contatoreColpiBoss] = Colpo(self, "boss", self.cheColpo+1, 30)
